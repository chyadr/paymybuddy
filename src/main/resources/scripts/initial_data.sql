            ...
            values (nextval('seq_ids'),'Bank','BANK','bank@paymybuddy.com','$2a$12$BOMd1ZPq/mlCNNKoJFwgv.uXKAaK0HWKfYn8ClYlrrckYD7PuthQq');
            ...
            insert into pay_transaction (id,user_id,connected_user_id,amount,description)
            values(nextval('seq_ids'),(select user_id from pay_connection where user_id in(select id from pay_user where first_name='David')),(select connected_user_id from pay_connection where user_id in(select id from pay_user where first_name='David')),20,'Restraunt');

            insert into pay_transaction (id,user_id,connected_user_id,amount,description)
            values(nextval('seq_ids'),(select user_id from pay_connection where user_id in(select id from pay_user where first_name='David')),(select connected_user_id from pay_connection where user_id in(select id from pay_user where first_name='David')),5,'Gift');
